...
QueryColumnsSQL(database, schema, table string) (string, []interface{})
// ScanColumnRow scans one row from the columns result-set.
ScanColumnRow(rows *sql.Rows) (Column, error)

// QueryViewDefinitionSQL returns a SQL statement and bind arguments
// that yield a single string: the DDL/body of the object (empty for
// plain tables).
QueryViewDefinitionSQL(database, schema, table string) (string, []interface{})
// ScanViewDefinition scans the single row returned by the view
// definition query.
ScanViewDefinition(row *sql.Row) (string, error)

// QueryDependenciesSQL returns a SQL statement and bind arguments for
// the foreign references of an object.
QueryDependenciesSQL(database, schema, table string) (string, []interface{})
...
// quoteMSSQL wraps an identifier in MSSQL bracket notation and escapes
// embedded closing brackets.
func quoteMSSQL(id string) string {
return "[" + strings.ReplaceAll(id, "]", "]]") + "]"
}

// quoteMySQL wraps an identifier in MySQL backtick notation and escapes
// embedded backticks.
func quoteMySQL(id string) string {
return "`" + strings.ReplaceAll(id, "`", "``") + "`"
}

// quoteStd wraps an identifier in standard double-quote notation (used by
// PostgreSQL and SQLite) and escapes embedded double-quotes.
func quoteStd(id string) string {
return `"` + strings.ReplaceAll(id, `"`, `""`) + `"`
}
...
func (mssqlDialect) QueryTablesSQL(database string) (string, []interface{}) {
...
qdb := quoteMSSQL(database)
qsch := quoteMSSQL(schema)
qtbl := quoteMSSQL(table)
q := fmt.Sprintf(
...
func (mssqlDialect) QueryViewDefinitionSQL(database, schema, table string) (string, []interface{}) {
qdb := quoteMSSQL(database)
qsch := quoteMSSQL(schema)
qtbl := quoteMSSQL(table)
q := fmt.Sprintf(
...
func (mssqlDialect) QueryDependenciesSQL(database, schema, table string) (string, []interface{}) {
qdb := quoteMSSQL(database)
qsch := quoteMSSQL(schema)
qtbl := quoteMSSQL(table)
q := fmt.Sprintf(
...
func (mysqlDialect) QueryTablesSQL(database string) (string, []interface{}) {
q := "SELECT TABLE_SCHEMA, TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE " +
...
return q, []interface{}{database, table}
}
...
func (mysqlDialect) QueryViewDefinitionSQL(database, _, table string) (string, []interface{}) {
q := "SELECT COALESCE(VIEW_DEFINITION, '') " +
...
// A plain table won't have a row in information_schema.VIEWS.
...
func (mysqlDialect) QueryDependenciesSQL(database, _, table string) (string, []interface{}) {
q := "SELECT COALESCE(REFERENCED_TABLE_SCHEMA, ''), " +
...
func (postgresDialect) QueryTablesSQL(_ string) (string, []interface{}) {
...
return q, []interface{}{schema, table}
}
...
func (postgresDialect) QueryViewDefinitionSQL(_, schema, table string) (string, []interface{}) {
q := "SELECT COALESCE(view_definition, '') " +
"FROM information_schema.views " +
"WHERE table_schema = $1 AND table_name = $2"
return q, []interface{}{schema, table}
...
func (postgresDialect) QueryDependenciesSQL(_, schema, table string) (string, []interface{}) {
q := "SELECT tc.table_catalog, ccu.table_schema, ccu.table_name " +
...
func (sqliteDialect) QueryTablesSQL(_ string) (string, []interface{}) {
q := "SELECT '' as catalog, '' as schema_name, name, type " +
"FROM sqlite_master " +
...
// QueryColumnsSQL returns a PRAGMA statement; SQLite does not use
// information_schema for column metadata. The table name is quoted as a
// SQL identifier to prevent injection.
func (sqliteDialect) QueryColumnsSQL(_, _, table string) (string, []interface{}) {
q := fmt.Sprintf("PRAGMA table_info(%s)", quoteStd(table))
return q, nil
}
...
func (sqliteDialect) QueryViewDefinitionSQL(_, _, table string) (string, []interface{}) {
q := "SELECT COALESCE(sql, '') FROM sqlite_master WHERE type='view' AND name=?"
return q, []interface{}{table}
}
...
// QueryDependenciesSQL returns a PRAGMA statement for foreign keys. The table
// name is quoted as a SQL identifier to prevent injection.
func (sqliteDialect) QueryDependenciesSQL(_, _, table string) (string, []interface{}) {
q := fmt.Sprintf("PRAGMA foreign_key_list(%s)", quoteStd(table))
return q, nil
}
